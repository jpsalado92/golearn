...
func searchTree(directoryPath string, filePathMetadataChan chan<- filePathMetadataType, wg *sync.WaitGroup) error {
	defer wg.Done()
	visitFunc := func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() && d.Name() != filepath.Base(directoryPath) {
			wg.Add(1)
			go searchTree(path, filePathMetadataChan, wg)
...
func run(dir string) results {
	...
	filePathMetaDataChan := make(chan filePathMetadataType, 20)
	...
	wg.Add(1)
	if err := searchTree(dir, filePathMetaDataChan, &wg); err != nil {
		log.Fatal(err)
	}
	wg.Wait()
